...
Helena Torres answered the full expert-feedback questionnaire. The questions and answers are included below.
...
> Pseudo-labeling is a reasonable strategy, particularly when deployment simplicity is important. Using a 90% confidence threshold is also a conservative choice that may reduce incorrect pseudo-labels. However, analysing the behaviour of the initial model at this threshold would help better understand possible biases, systematic errors, and issues with visually similar classes.
...
> The architecture is "defensible" if edge deployment and inference efficiency are priorities. However, I would still compare against lightweight architectures such as MobileNetV3 or EfficientNet-Lite.
...
> Both intensity-based augmentations (such as noise addition, color jitter, and contrast/brightness changes) and spatial augmentations (such as flips, rotations, and deformable transformations) are important when labeled data is limited. However, in medical imaging tasks, deformable transformations should be used carefully, as they may distort clinically meaningful anatomical structures.
...
> I would definitely test multiple resolutions to understand the trade-off between accuracy and inference time.
...
> I consider per-class precision/recall, F1-score and confusion matrices essential for classification tasks, because using only global accuracy can be misleading.
...
> I would first inspect dataset quality and visually assess class separability. Another important aspect to analyze is class imbalance, since it can strongly influence confusion between similar classes. Additionally, I would compare the model's performance using different input resolutions to evaluate whether the confusion could be reduced by providing more visual detail to the network.
...
> You could use fixed reference images and compare the outputs numerically across different devices and frameworks to validate preprocessing and inference consistency.
...
> Portability is very important for some projects, although it can also be challenging to achieve consistently across platforms. In the projects where deployment portability is required, we usually bridge this gap by training models in Python/PyTorch and then exporting them through frameworks such as ONNX for deployment in C++ or mobile environments.
...
> From a clinical perspective, this is can be important because new diagnostic categories may need to be integrated over time. For example, a model may initially classify cases only as normal or abnormal, but later need to evolve to distinguish between normal, abnormal type 1, and abnormal type 2 without forgetting previously learned categories.
...
> I have not worked extensively on catastrophic forgetting mitigation, mainly because most of our projects were clinically well-defined from the beginning and did not require frequent integration of new classes. However, based on the literature and state of the art, rehearsal buffer approaches seem to be among the most robust.
...
> I think this depends strongly on the application. In some medical imaging scenarios, 80 ms may still be not fully real time, while in others it would already be perceived as real time. However, for a plant disease detection application where users capture or analyze one image at a time, this latency is more than acceptable in my opinion.
...
> Memory constraints while maintaining model accuracy.
...
> I would test first segmentation-assisted classification to improve explainability and robustness.
...
> Absolutely. Semi-supervised learning and edge deployment can be highly relevant in medical imaging, such as for point-of-care devices, and industrial inspection.
