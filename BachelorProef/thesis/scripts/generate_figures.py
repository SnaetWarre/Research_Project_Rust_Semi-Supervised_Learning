...
def figure_deployment_comparison() -> None:
    ...
    inner = [
        ("Burn runtime", 0.40),
        ("Model weights\n(~5 MB)", 0.27),
        ("Application code", 0.33),
    ]
    cy = by
    for label, frac in inner:
        h = bh * frac
        draw_box(
            ax,
            bx + 3,
...
def figure_pipeline_flowchart() -> None:
    fig, ax = plt.subplots(figsize=mm_to_inches(160, 105))
    strip_axes(ax, (0, 160), (0, 105))
    ...
    center_x = 80
    node(56, 92, 48, 9, "Start: labeled data (20%)", BLUE)
    node(56, 79, 48, 9, "Train initial CNN")
    node(54, 66, 52, 9, "Inference on unlabeled stream")
    diamond(center_x, 53, 45, 14, "Confidence >= 0.9?")
    node(56, 38, 48, 8, "Accept as pseudo-label")
    node(116, 49, 34, 9, "Reject / discard")
    diamond(center_x, 28, 42, 14, "Buffer >= 200?")
    node(16, 24, 44, 9, "Retrain CNN on labeled + pseudo")
    diamond(38, 11, 38, 12, "Validation plateau?")
    node(108, 7, 36, 9, "Final model", BLUE)

    arrow(ax, (80, 92), (80, 88))
    arrow(ax, (80, 79), (80, 75))
    arrow(ax, (80, 66), (80, 60))
    arrow(ax, (80, 46), (80, 45))
    ax.text(89, 47, "Yes", fontsize=7, color=BLUE)
    arrow(ax, (102, 53), (116, 53))
    ax.text(107, 56, "No", fontsize=7, color=GRAY)
    arrow(ax, (80, 38), (80, 35))
    arrow(ax, (59, 28), (60, 28))
    ax.text(61, 31, "Yes", fontsize=7, color=BLUE)
    arrow(ax, (38, 24), (38, 17))
    arrow(ax, (57, 11), (108, 11), color=BLUE)
    ax.text(72, 13, "Yes", fontsize=7, color=BLUE)
    arrow(ax, (20, 11), (20, 70), color=GRAY, dashed=True)
    arrow(ax, (20, 70), (54, 70), color=GRAY, dashed=True)
    ax.text(23, 70.5, "No", fontsize=7, color=GRAY)
    arrow(ax, (101, 28), (121, 66), color=GRAY, dashed=True)
    arrow(ax, (121, 66), (106, 70), color=GRAY, dashed=True)
    ax.text(104, 30, "No", fontsize=7, color=GRAY)
...
def figure_forgetting_strategies() -> None:
    ...
    for x, title in zip(col_x, titles):
        ax.text(x + 19, 81, title, ha="center", weight="bold", fontsize=11, color=BLUE if title == "Rehearsal" else TEXT)
        draw_box(ax, x, 18, 38, 56, "", face="white", edge=GRID, lw=1.0)
    ...
    ax.text(col_x[0] + 19, 37, "Protect important weights", ha="center", fontsize=7.5, color=RED)
    ax.text(col_x[0] + 19, 29, "EWC: protect weights\nLwF: distil old outputs", ha="center", va="center", fontsize=7.2)

    # Rehearsal: buffer feeding training loop.
    bx, by = col_x[1] + 5, 52
    for offset in [0, 3, 6]:
        draw_box(ax, bx + offset, by - offset, 14, 10, "", face="#F7F7F7", edge=LIGHT_GRAY)
    draw_box(ax, col_x[1] + 20, 49, 14, 10, "Train", face=BLUE, edge=BLUE, color="white", size=7)
    arrow(ax, (col_x[1] + 20, 54), (col_x[1] + 19, 54), color=BLUE)
    arrow(ax, (col_x[1] + 27, 49), (col_x[1] + 12, 45), color=BLUE, dashed=True)
    arrow(ax, (col_x[1] + 12, 45), (col_x[1] + 10, 52), color=BLUE, dashed=True)
    ax.text(col_x[1] + 19, 37, "Memory buffer replay", ha="center", fontsize=7.5, color=BLUE)
    ax.text(col_x[1] + 19, 29, "Replay old examples\nGEM: constrain updates", ha="center", va="center", fontsize=7.2)
    ...
    ax.text(col_x[2] + 19, 37, "Add or freeze capacity", ha="center", fontsize=7.5, color=GREEN)
    ax.text(col_x[2] + 19, 29, "Progressive Nets: add columns\nPackNet: prune and freeze", ha="center", va="center", fontsize=7.2)
...
def figure_label_efficiency_curve() -> None:
    ...
    save(fig, REPO_ROOT / "plantvillage_ssl/output/experiments/label_efficiency/label_efficiency_curve.svg")
...
def figure_label_efficiency_bars() -> None:
    ...
    save(fig, REPO_ROOT / "plantvillage_ssl/output/experiments/label_efficiency/label_efficiency_bars.svg")
...
def figure_class_scaling() -> None:
    ...
    save(fig, REPO_ROOT / "plantvillage_ssl/output/experiments/class_scaling/class_scaling_comparison.svg")
...
def figure_new_class_accuracy() -> None:
    ...
    save(fig, REPO_ROOT / "plantvillage_ssl/output/experiments/new_class_position/new_class_accuracy_curve.svg")
...
def figure_position_comparison() -> None:
    ...
    save(fig, REPO_ROOT / "plantvillage_ssl/output/experiments/new_class_position/position_comparison_50.svg")
...
def figure_forgetting_curve() -> None:
    ...
    save(fig, REPO_ROOT / "plantvillage_ssl/output/experiments/new_class_position/forgetting_curve.svg")
