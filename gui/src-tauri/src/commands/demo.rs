...
impl Default for DemoConfig {
    fn default() -> Self {
        Self {
            ...
            // FIXED: 0.2 = 20% labeled, leaving 60% for SSL stream
            // Previously was 0.9 which left only 10% for streaming (too little!)
            labeled_ratio: 0.2,
            seed: 42,
        }
    }
...
pub struct DayImage {
    ...
    pub is_farmer_image: bool,            // Whether this is from farmer demo
}
...
pub struct DemoSession<B: AutodiffBackend> {
    ...
    farmer_simulator: Option<StreamSimulator>, // Farmer demo images (from stream pool, never in training)
...
pub async fn advance_demo_day(
    ...
    let result = process_demo_day::<AdaptiveBackend>(session, &app)
        .map_err(|e| {
...
pub async fn manual_retrain_demo(
    ...
    let _ = app.emit("demo:retraining_data_sizes", serde_json::json!({
        "labeled_pool_size": session.splits.labeled_pool.len(),
        "pseudo_labels_size": pseudo_labels_used,
    ...
    tracing::info!(
        "Starting retraining with {} total samples, 1 epoch (mobile optimized for speed)",
    ...
    B: AutodiffBackend,
{
    ...
    // Load farmer demo images first to know which ones to exclude from stream
    let farmer_images_result = load_farmer_demo_images(&config.data_dir);
    
    // Create a set of farmer image filenames for exclusion from main stream
    let farmer_filenames: std::collections::HashSet<String> = farmer_images_result
        .as_ref()
        .map(|images| {
            images
                .iter()
                .filter_map(|img| img.image_path.file_name())
                .map(|name| name.to_string_lossy().to_string())
                .collect()
        })
        .unwrap_or_default();

    // Filter stream pool to exclude farmer demo images (they'll be processed separately)
    let filtered_stream_pool: Vec<_> = if !farmer_filenames.is_empty() {
        unlabeled_pool
            .into_iter()
            .filter(|img| {
                img.image_path
                    .file_name()
                    .map(|name| !farmer_filenames.contains(&name.to_string_lossy().to_string()))
                    .unwrap_or(true)
            })
            .collect()
    } else {
    ...
    tracing::info!(
        "Stream pool: {} images (after excluding {} farmer demo images)",
        filtered_stream_pool.len(),
        farmer_filenames.len()
    );

    // Create stream simulator with filtered pool
    let stream_simulator = StreamSimulator::new(filtered_stream_pool, config.seed, config.images_per_day);

    // Create farmer simulator if images were found
    let farmer_simulator = match farmer_images_result {
    ...
    // Calculate total available images (farmer + stream)
    let farmer_count = farmer_simulator.as_ref().map(|s| s.total()).unwrap_or(0);
    let stream_count = stream_simulator.total();
    let total_available = farmer_count + stream_count;
...
fn process_demo_day<B: AutodiffBackend>(session: &mut DemoSession<B>, app: &AppHandle) -> Result<DayResult, String> {
    use std::time::Instant;
    ...
    if should_retrain {
        ...
        let _ = app.emit("demo:retraining_data_sizes", serde_json::json!({
            "labeled_pool_size": session.splits.labeled_pool.len(),
            "pseudo_labels_size": pseudo_labels.len(),
            "total_training_samples": combined_samples.len()
        }));
        ...
        tracing::info!(
            "Starting retraining with {} total samples, 5 epochs (mobile optimized for stability)",
            combined_dataset.len()
        );
    ...
    retrain_model_with_augmentation(
        ...
        // Evaluate after retraining
        let new_acc = evaluate_model(&session.model, &session.val_dataset, &session.batcher, 16);
...
/// Creates a fresh optimizer each time to avoid momentum buildup from previous training
fn retrain_model_with_augmentation<B: AutodiffBackend>(
    model: &mut PlantClassifier<B>,
    _optimizer: &mut burn::optim::adaptor::OptimizerAdaptor<
        burn::optim::Adam,
        PlantClassifier<B>,
        B,
    >,
    ...
    // CRITICAL FIX: Create a fresh optimizer for each retraining cycle
    // This prevents momentum accumulation from previous training which causes
    // the model to "remember" old gradients and drift away from the current data
    let mut fresh_optimizer = AdamConfig::new()
        .with_weight_decay(Some(burn::optim::decay::WeightDecayConfig::new(1e-4f32)))
        .init();
    ...
    for epoch in 0..epochs {
        ...
        for start in (0..len).step_by(batch_size) {
            ...
            if let Some(ref handle) = app_handle {
                let _ = handle.emit("demo:training_progress", serde_json::json!({
                    "epoch": epoch + 1,
                    "total_epochs": epochs,
                    "batch": batch_count,
                    "total_batches": total_batches,
                    "loss": loss_value,
                    "progress_percent": (batch_count as f64 / total_batches as f64 * 100.0).round() as u32
                }));
            }
            ...
            *model = fresh_optimizer.step(learning_rate, model.clone(), grads);
        }
