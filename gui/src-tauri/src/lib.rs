...
pub fn run() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter("plantvillage_gui=debug,demo=debug,info")
        .with_target(true)
...
#[tauri::command]
fn get_bundled_model_path(app: tauri::AppHandle) -> Result<String, String> {
    bundled_resources::get_bundled_model_path(&app)
        .map(|p| p.to_string_lossy().to_string())
}

/// Get the path to the bundled training data
#[tauri::command]
fn get_bundled_data_path(app: tauri::AppHandle) -> Result<String, String> {
    bundled_resources::get_bundled_data_path(&app)
        .map(|p| p.to_string_lossy().to_string())
}

/// List all available bundled resources
#[tauri::command]
fn list_bundled_resources(app: tauri::AppHandle) -> Result<Vec<String>, String> {
    bundled_resources::list_bundled_resources(&app)
}
