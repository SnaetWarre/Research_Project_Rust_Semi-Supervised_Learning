    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
            <div class="flex items-start justify-between">
                ...
                <div
                    class="w-12 h-12 rounded-lg {$modelInfo.loaded
                        ? 'bg-emerald-500/20'
                        : 'bg-slate-500/20'} flex items-center justify-center"
                >
                    <Brain
                        class="w-6 h-6 {$modelInfo.loaded
                            ? 'text-emerald-400'
                            : 'text-slate-400'}"
                    />
                </div>
        ...
        <Card>
            <div class="flex items-start justify-between">
                ...
                <div
                    class="w-12 h-12 rounded-lg {$trainingState.status ===
                    'running'
                        ? 'bg-yellow-500/20'
                ...
                >
                    <Activity
                        class="w-6 h-6 {$trainingState.status === 'running'
                ...
                <div class="space-y-3 max-h-80 overflow-y-auto pr-2">
                    {#each $datasetInfo.classNames.slice(0, 10) as className, i}
                        <div class="flex items-center gap-3">
                            <div
                                class="w-24 truncate text-sm text-slate-300"
                                title={className}
                            >
                                {className.replace(/_/g, " ")}
                            </div>
                            <div
                                class="flex-1 h-2 bg-background-lighter rounded-full overflow-hidden"
                            >
                                <div
                                    class="h-full bg-primary rounded-full"
                                    style="width: {($datasetInfo.classCounts[
                                        i
                                    ] /
            ...
            <div class="space-y-3 max-h-80 overflow-y-auto pr-2">
                ...
                {:else}
                    {#each $activityLog as item}
                        <div
                            class="flex items-start gap-3 py-2 border-b border-slate-700/50 last:border-0"
                        >
                            <svelte:component
                                this={getActivityIcon(item.type)}
                                class="w-5 h-5 mt-0.5 {getActivityColor(
                                    item.type,
                                )}"
                            />
