...
impl PseudoLabeler {
    ...
    /// Also resets class counts to allow fresh pseudo-label collection after retraining
    pub fn get_and_clear_pseudo_labels(&mut self) -> Vec<PseudoLabeledImage> {
        let labels = std::mem::take(&mut self.pseudo_labels);
        // CRITICAL: Reset class counts after retraining to allow new pseudo-labels
        // Without this, max_per_class limits would prevent accepting new samples
        self.class_counts.clear();
        labels
    }

    /// Get all accumulated pseudo-labels and clear the buffer, but keep class counts
    /// Use this when you want to preserve class balance tracking across sessions
    #[allow(dead_code)]
    pub fn get_and_clear_pseudo_labels_keep_counts(&mut self) -> Vec<PseudoLabeledImage> {
        std::mem::take(&mut self.pseudo_labels)
    }
